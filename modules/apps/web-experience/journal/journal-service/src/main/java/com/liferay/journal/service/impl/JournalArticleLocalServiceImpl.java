...
public class JournalArticleLocalServiceImpl
	...
	protected void notifySubscribers(
		...
		try {
			...
			articleDiffs = JournalApiUtil.diffHtml(
				article.getGroupId(), article.getArticleId(),
	...
	protected BaseModelSearchResult<JournalArticle> searchJournalArticles(
		...
		for (int i = 0; i < 10; i++) {
			...
			List<JournalArticle> articles = JournalApiUtil.getArticles(hits);
	...
	protected void validateDDMStructureId(
		...
		int restrictionType = JournalApiUtil.getRestrictionType(folderId);
	...
	protected void validateReferences(
		...
		if (Validator.isNotNull(layoutUuid)) {
			Layout layout = JournalApiUtil.getArticleLayout(
				layoutUuid, groupId);
