...
<c:choose>
	<c:when test="<%= themeDisplay.isSignedIn() %>">
		...
		if (themeDisplay.isShowMyAccountIcon() && (themeDisplay.getURLMyAccount() != null)) {
			...
			sb.append("<a class=\"signed-in\" href=\"");
			sb.append(HtmlUtil.escape(String.valueOf(themeDisplay.getURLMyAccount())));
			sb.append("\">");
			sb.append(signedInAs);
			sb.append("</a>");

			signedInAs = sb.toString();
		}
		%>
			...
			<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();">
				...
				<c:choose>
					...
					<c:when test='<%= SessionMessages.contains(request, "userAdded") %>'>
						...
						<div class="alert alert-success">
							<c:choose>
								<c:when test="<%= Validator.isBlank(userPassword) %>">
				...
				<aui:fieldset>
					...
					<aui:input autoFocus="<%= windowState.equals(LiferayWindowState.EXCLUSIVE) || windowState.equals(WindowState.MAXIMIZED) %>" cssClass="clearable" label="<%= loginLabel %>" name="login" showRequiredLabel="<%= false %>" required="true" type="text" value="<%= login %>" />

					<aui:input name="password" showRequiredLabel="<%= false %>" type="password" required="true" value="<%= password %>" />
