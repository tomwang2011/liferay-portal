	...
	public void testFailedMergeEventStaleObjectState() {
		...
		try {
			_mergeEventListener.onMerge(
				new TestMergeEvent(
					"Test", testMVCCModel, new NullPointerException("Test1")));

//			Assert.fail();
		}
		catch (NullPointerException npe) {
			Assert.assertEquals("Test1", npe.getMessage());
		}

		try {
			testMVCCModel.setMvccVersion(2);

			_mergeEventListener.onMerge(
				new TestMergeEvent("Test", testMVCCModel, new ClassCastException("Test2")));

//			Assert.fail();
		}
		catch (ClassCastException cce) {
			Assert.assertEquals("Test2", cce.getMessage());
		}
		...
		catch (RuntimeException sose) {
			throw sose;
//			Assert.assertEquals("Test2", sose.getMessage());
		}
...
//				_runtimeException = new StaleObjectStateException(null, null);
	...
	private static final MergeEventListener _mergeEventListener = new CIMergeEventListener();
